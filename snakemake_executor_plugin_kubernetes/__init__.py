...
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % nano __init__.py 
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % nano __init__.py
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % nano __init__.py
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % nano __init__.py
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % nano __init__.py
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % cp nano __init__.py copy_init.py
cp: copy_init.py is not a directory
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % mv __init__.py copy_init.py
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % nano __init__.py
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % rm __init__.py 
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % nano __init__.py
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % rm __init__.py              
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % nano __init__.py
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % nano __init__.py                                                                                
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % pwd
/opt/anaconda3/envs/snakemake8/lib/python3.12/site-packages/snakemake_executor_plugin_kubernetes
(base) devinburke@dburkeMacbookPro snakemake_executor_plugin_kubernetes % cat __init__.py 
import base64
from dataclasses import dataclass, field
from pathlib import Path
import shlex
...
class Executor(RemoteExecutor):
    ...
    def run_job(self, job: JobExecutorInterface):
        ...
        if 'gpu' in job.resources:
            ...
            self.logger.debug(f"Added toleration for GPU nodes: {body.spec.tolerations}")
        ...
        container.resources.limits["cpu"] = "{}m".format(
            int(job.resources["_cores"] * 1000)
        )
        if "mem_mb" in job.resources.keys():
            container.resources.requests["memory"] = "{}M".format(
                job.resources["mem_mb"]
            )
            container.resources.limits["memory"] = "{}M".format(
                job.resources["mem_mb"]
            )
        if "disk_mb" in job.resources.keys():
            disk_mb = int(job.resources.get("disk_mb", 1024))
